...
AvalancheMC_My::AvalancheMC_My() { m_drift.reserve(10000); }

void AvalancheMC_My::SetSensor(Sensor* sensor) {
  if (!sensor) {
...
void AvalancheMC_My::SetCollisionSteps(const unsigned int n) {
...
void AvalancheMC_My::SetStepDistanceFunction(double (*f)(double x, double y,
                                                      double z)) {
  if (!f) {
...
void AvalancheMC_My::GetDriftLinePoint(const size_t i, double& x, double& y,
...
void AvalancheMC_My::GetHoleEndpoint(const size_t i, double& x0, double& y0,
...
void AvalancheMC_My::GetIonEndpoint(const size_t i, double& x0, double& y0,
...
void AvalancheMC_My::GetElectronEndpoint(const size_t i, double& x0,
...
bool AvalancheMC_My::DriftPositron(const double x0, const double y0,
  ...
  std::vector<DriftPoint> secondaries;
  return DriftLine({x0, y0, z0}, t0, Particle::Positron, secondaries);
}

bool AvalancheMC_My::DriftElectronBothWays(const double x0, const double y0, const double z0,
                const double t0) {
  bool ok1 = DriftPositron(x0, y0, z0, t0);
  bool ok2 = DriftElectron(x0, y0, z0, t0);
  return ok1 && ok2;
}
...
void AvalancheMC_My::WriteFieldAlongDriftLine (std::ostream& str, double min_dl, double &L)
{
  ...
  std::array<double, 3> e_sum = {0., 0., 0.};
  std::array<double, 3> b = {0., 0., 0.};
  Medium* medium = nullptr;
  double ls=L, dl=0;
  int n_averaged=0;
  for (auto i = m_drift.begin(); i!=_end_; ++i) {
    dl +=(i==_begin_) ? 0 : sqrt(pow(i->x[0]-(i-1)->x[0], 2) +
        pow(i->x[1]-(i-1)->x[1], 2) + pow(i->x[2]-(i-1)->x[2], 2));
    ls += dl;
    GetField(i->x, e, b, medium);
    e_sum[0] += e[0];
    e_sum[1] += e[1];
    e_sum[2] += e[2];
    ++n_averaged;
    if (dl>=min_dl) {
      str<<ls<<"\t"<<i->x[0]<<"\t"<<i->x[1]<<"\t"<<i->x[2]
	       <<"\t"<<e_sum[0]/n_averaged<<"\t"<<e_sum[1]/n_averaged<<"\t"<<e_sum[2]/n_averaged
         <<"\t"<<sqrt(e_sum[0]*e_sum[0]+e_sum[1]*e_sum[1]+e_sum[2]*e_sum[2])/n_averaged<<std::endl;
      dl = 0;
      n_averaged = 0;
      e_sum[0] = e_sum[1] = e_sum[2] = 0;
    }
  }
  if(dl!=0){
    str<<ls<<"\t"<<m_drift.back().x[0]<<"\t"<<m_drift.back().x[1]<<"\t"<<m_drift.back().x[2]
       <<"\t"<<e_sum[0]/n_averaged<<"\t"<<e_sum[1]/n_averaged<<"\t"<<e_sum[2]/n_averaged
       <<"\t"<<sqrt(e_sum[0]*e_sum[0]+e_sum[1]*e_sum[1]+e_sum[2]*e_sum[2])/n_averaged<<std::endl;
  }
  L=ls;
...
bool AvalancheMC_My::DriftHole(const double x0, const double y0, const double z0,
                            const double t0) {
  if (!m_sensor) {
...
bool AvalancheMC_My::DriftIon(const double x0, const double y0, const double z0,
                           const double t0) {
  if (!m_sensor) {
...
bool AvalancheMC_My::DriftNegativeIon(const double x0, const double y0,
                                   const double z0, const double t0) {
  if (!m_sensor) {
...
bool AvalancheMC_My::DriftLine(const std::array<double, 3>& xi, const double ti,
  ...
  if ((particle == Particle::Electron || particle == Particle::Positron || particle == Particle::Hole) &&
...
void AvalancheMC_My::AddElectron(const double x, const double y, const double z,
                              const double t) {
  AddEndPoint({x, y, z}, t, {x, y, z}, t, StatusAlive, Particle::Electron);
  ++m_nElectrons;
}

void AvalancheMC_My::AddHole(const double x, const double y, const double z,
                          const double t) {
  AddEndPoint({x, y, z}, t, {x, y, z}, t, StatusAlive, Particle::Hole);
  ++m_nHoles;
}
void AvalancheMC_My::AddIon(const double x, const double y, const double z,
                         const double t) {
  AddEndPoint({x, y, z}, t, {x, y, z}, t, StatusAlive, Particle::Ion);
  ++m_nIons;
}

bool AvalancheMC_My::ResumeAvalanche(const bool electrons, const bool holes) {
...
bool AvalancheMC_My::Avalanche(std::vector<DriftPoint>& aval,
...
int AvalancheMC_My::GetField(const std::array<double, 3>& x,
...
double AvalancheMC_My::GetMobility(const Particle particle, Medium* medium) const {
...
bool AvalancheMC_My::GetVelocity(const Particle particle, Medium* medium,
...
bool AvalancheMC_My::GetDiffusion(const Particle particle, Medium* medium,
                               const std::array<double, 3>& e,
...
double AvalancheMC_My::GetAttachment(const Particle particle, Medium* medium,
                                  const std::array<double, 3>& x,
                                  const std::array<double, 3>& e,
...
double AvalancheMC_My::GetTownsend(const Particle particle, Medium* medium,
                                const std::array<double, 3>& x,
                                const std::array<double, 3>& e,
                                const std::array<double, 3>& b) const {
  double alpha = 0.;
...
void AvalancheMC_My::StepRKF(const Particle particle,
...
void AvalancheMC_My::AddDiffusion(const double step, const double dl,
...
void AvalancheMC_My::Terminate(const std::array<double, 3>& x0, const double t0,
...
bool AvalancheMC_My::ComputeGainLoss(const Particle particle,
...
bool AvalancheMC_My::ComputeAlphaEta(const Particle particle,
                                  std::vector<DriftPoint>& driftLine,
                                  std::vector<double>& alps,
...
bool AvalancheMC_My::Equilibrate(std::vector<double>& alphas) const {
...
void AvalancheMC_My::ComputeSignal(
...
void AvalancheMC_My::ComputeInducedCharge(
...
void AvalancheMC_My::PrintError(const std::string& fcn, const std::string& par,
                             const Particle particle,
