...
def prepare_settings(V0, Vth1):
    ...
    if field_map_files is None:
        return None
    RecalculateMesh = False # Needs to be recalulated only once

    abs_output_path = os.path.normpath(os.path.join(dir_path, ResultsFolder))
    abs_settings = os.path.normpath(os.path.join(abs_output_path, "settings_"+str(round(V0, 1)) + "V.xml"))
...
if __name__ == "__main__":
    for i in range(min(len(V0s), len(Vt1s))):
        ...
        completed_process = subprocess.run([binary, files[1]], \
            check=False, cwd=abs_binary_path)
